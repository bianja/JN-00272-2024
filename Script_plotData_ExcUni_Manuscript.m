...
switch type
    case 'excuni2'
%         load('\\132.187.28.171\home\Manuskript\I Optic flow\data\RF\Results_ExcUni2_addRF_new_extended.mat')
        load('\\132.187.28.171\home\rest\Manuskript\I_Optic_flow\JEB\data\RF\Results_ExcUni2_addRF_new_extended.mat')
    case 'excuni1'
%         load('\\132.187.28.171\home\Manuskript\I Optic flow\data\RF\Results_ExcUni1_addRF_new_extended.mat')
...
set(gca,'XScale','log','xtick',[0 10 100],'xticklabels',{'0','10','100'})
% % for ft
ylim([-.2 1])
% xlim([10^0.6 10^3.6])
xlim([0 140]) % for ft
...
switch type
    case 'excuni2'
        print(['ExcUni2_TC'],'-depsc','-r300','-tiff','-painters')
        savefig(['ExcUni2_TC.fig'])
    case 'excuni1'
        print(['ExcUni1_TC'],'-depsc','-r300','-tiff','-painters')
        savefig(['ExcUni1_TC.fig'])
end
...
for i = 1 : size(AllAni,2)
    ...
    normbg = 0;%AllAni(i).R01.background.sum(1)/maxval;
...
switch type
    case 'excuni2'
        print('ExcUni2_heatmap','-depsc','-r300','-tiff','-painters')
        savefig('ExcUni2_heatmap.fig')
    case 'excuni1'
        print('ExcUni1_heatmap','-depsc','-r300','-tiff','-painters')
        savefig('ExcUni1_heatmap.fig')
end
...
switch type
    case 'excuni2'
        print('ExcUni2_heatmap_mean','-depsc','-r300','-tiff','-painters')
        savefig('ExcUni2_heatmap_mean.fig')
    case 'excuni1'
        print('ExcUni1_heatmap_mean','-depsc','-r300','-tiff','-painters')
        savefig('ExcUni1_heatmap_mean.fig')
end
...
switch type
    case 'excuni2'
        print(['ExcUni2_bp_btf'],'-depsc','-r300','-tiff','-painters')
        savefig(['ExcUni2_bp_btf.fig'])
    case 'excuni1'
        print(['ExcUni1_bp_ftb'],'-depsc','-r300','-tiff','-painters')
        savefig(['ExcUni1_bp_ftb.fig'])
end
...
switch type
    case 'excuni2'
        print(['ExcUni2_bp_ftb'],'-depsc','-r300','-tiff','-painters')
        savefig(['ExcUni2_bp_ftb.fig'])
    case 'excuni1'
        print(['ExcUni1_bp_btf'],'-depsc','-r300','-tiff','-painters')
        savefig(['ExcUni1_bp_btf.fig'])
end
...
switch type
    case 'excuni2'
        ...
        save ExcUni2_BP_stats.mat statsftb statsbtf resultsftb resultsbtf statsPftb statsPbtf
    case 'excuni1'
        [statsftb,~,stats] = friedman(temp_I); close;
        resultsftb = multcompare(stats,'CType','bonferroni'); close;
        [statsbtf,~,stats] = friedman(temp_E); close;
        resultsbtf = multcompare(stats,'CType','bonferroni'); close;
        
        for i = 1 : 3
            for j = 1 : 3
                statsPftb(i,j) = signrank(temp_I(:,i),temp_I(:,j));
                statsPbtf(i,j) = signrank(temp_E(:,i),temp_E(:,j));
            end
        end
        save ExcUni1_BP_stats.mat statsftb statsbtf resultsftb resultsbtf statsPftb statsPbtf
end
